...
## 33. CRM — bug crítico: `whatsapp-webhook` travando em todo boot (`time is not defined`)

**Incidente (22-23/09/2026).** Disparo de campanha via API oficial do
WhatsApp (número terminado em 3737) saiu normalmente, mas o time
comercial reportou que as respostas dos leads não apareciam no
Kanban. Investigando os **Logs** do Edge Function `whatsapp-webhook`
(Supabase → Edge Functions → Logs), apareceu em **todo boot** da
função um `UncaughtException`:

```
event loop error: ReferenceError: time is not defined
    at file:///var/tmp/sb-compile-edge-runtime/source/index.ts:115:125
```

Isso acontecia mesmo com os secrets `VAPID_PUBLIC_KEY`/
`VAPID_PRIVATE_KEY` **não configurados** (push já aparecia como
"desativado" nos logs), ou seja, o crash não vinha de chamarmos
`webpush.setVapidDetails` (isso já tinha sido corrigido em 16/09/2026,
ver comentário no código do §18.4) — vinha do próprio **import** do
pacote `npm:web-push@3.6.7`, que dispara algo internamente
independente de usarmos a lib ou não. Coluna 125 de uma única linha
indica código minificado/empacotado (dependência do `web-push`), não
código nosso.

**Correção:** removido o `import webpush from "npm:web-push@3.6.7"` e
toda a feature de push notification (consts `VAPID_*`, bloco
`pushHabilitado`, função `notificarPush` e a chamada dela no loop de
mensagens recebidas) do `whatsapp-webhook`. A feature não estava nem
ativa em produção (faltavam os secrets), então nada foi perdido — só
removida a causa do crash. Resto da função (gravar mensagem, marcar
...
> **Nota (23/09/2026):** notificação push (aviso no celular de nova
> mensagem) ficou removida do CRM por enquanto — era um "extra" que
